      ...
      if(map:contains($optionsMap, "all") or map:contains($optionsMap, "amps")) then
        for $amp in
          xdmp:eval('
            ...
            let $db-exists :=
	      if (fn:string($amp/sec:database) ne "0") then
                try{
                  let $_ := xdmp:database-name(xs:unsignedLong($amp/sec:database))
                  return fn:true()
                } catch($ignore){
                  fn:false()
                }
    ...
    return
      xdmp:eval(
        ...
        (xs:QName("namespace"), $amp/sec:namespace,
         ...
         xs:QName("database"), if ($amp/sec:database-name eq "filesystem") then xs:unsignedLong(0) else xdmp:database($amp/sec:database-name),
         xs:QName("role-name"), $role-name),
        $eval-options),
  ...
  return
    ...
    (
      xdmp:eval(
        'import module namespace sec="http://marklogic.com/xdmp/security" at "/MarkLogic/security.xqy";
         declare variable $amp external;
         declare variable $db := if($amp/sec:db-name = "filesystem") then xs:unsignedLong(0) else xdmp:database($amp/sec:db-name);
         sec:amp-set-roles(
           $amp/sec:namespace,
           $amp/sec:local-name,
           $amp/(sec:doc-uri, sec:document-uri)[1],
    ...
    <amps xmlns="http://marklogic.com/xdmp/security">
    {
      ...
      return
        element sec:amp
        {
          ...
          element sec:db-name
          {
            if (xs:unsignedLong($amp/sec:database) = xs:unsignedLong(0)) then "filesystem"
            else
              try {
                xdmp:database-name(xs:unsignedLong($amp/sec:database))
              } catch ($e) {
...
{
  (: Handles both old format (9.0-12) and new ML11 format (11.2.0) :)
